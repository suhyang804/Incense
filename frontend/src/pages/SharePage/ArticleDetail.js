...
function ArticleDetail() {
  ...
  const onChangeSecret = () => {
    isSecret === 0 ? setIsSecret(1) : setIsSecret(0);
  };
  ...
  if (article.imageInfo) {
    for (let i = 0; i < article.imageInfo.length; i++) {
      images.push({
        url: `https://j8a804.p.ssafy.io/api/display?filename=${article.imageInfo[i]}`,
      });
    }
  }
  ...
  const onSubmitComment = () => {
    if (isLoggedIn === true) {
      api.comment
        .register(articleId, commentRegister)
        .then((res) => {
          window.location.reload();
        })
        .catch((err) => {
  ...
  return (
    <Box sx={{ width: "60%", margin: "1rem auto", marginBottom: "5rem" }}>
        ...
        <Box sx={{ width: "60%", display: "flex", flexDirection: "column" }}>
          ...
          <Box sx={{ width: "100%", display: "flex", flexDirection: "row" }}>
            ...
            >
              ...
              <P3>
                구매시기 : {article.buyDate.substr(0, 4)} 년{" "}
                {article.buyDate.substr(5)} 월
              </P3>
            </Box>
            <Box
              sx={{ width: "50%", display: "flex", flexDirection: "column" }}
      ...
      <h1
        style={{ marginLeft: "1rem", fontSize: "1.3rem", fontWeight: "bold" }}
      >
        댓글
      </h1>
      <Box
